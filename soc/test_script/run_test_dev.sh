...
do
	...
	echo INFO:t_standard=
	for val in ${result_standard[*]}
	do
		echo $val
	done
	echo
	...
	do
		...
		then
			...
			flag=0		
			echo ERROR: Result$i is wrong.; echo
			break
		fi
	done
	...
	if [ $flag -eq 1 ]
	then
		echo INFO:Result is correct!
	fi

	echo SAMPLE$loop END
	echo "******************************"
done
